...
const seed = (...tasks: string[]) =>
    tasks.map((task, i) =>
        taskService.addTask(USER, { task, completed: false }),
    );
...
describe('addTask', () => {
    it('creates a task and returns its id', () => {
        const id = taskService.addTask(USER, { task: 'buy milk', completed: false });
...
describe('getTasks', () => {
    ...
    it('does not return tasks belonging to another user', () => {
        taskService.addTask(OTHER_USER, { task: 'other task', completed: false });
...
describe('getTask', () => {
    it('returns the task by id for the correct user', () => {
        const id = taskService.addTask(USER, { task: 'hello', completed: false });
    ...
    it('returns undefined for a task owned by another user', () => {
        const id = taskService.addTask(OTHER_USER, { task: 'secret', completed: false });
...
describe('updateTask', () => {
    it('updates task text and completed status', () => {
        const id = taskService.addTask(USER, { task: 'old text', completed: false });
    ...
    it('returns false when task belongs to another user', () => {
        const id = taskService.addTask(OTHER_USER, { task: 'theirs', completed: false });
...
describe('deleteTask', () => {
    it('deletes an existing task and returns true', () => {
        const id = taskService.addTask(USER, { task: 'remove me', completed: false });
...
describe('orderTasks', () => {
    ...
    it('returns false when a task id does not belong to the user', () => {
        seed('mine');
        const otherId = taskService.addTask(OTHER_USER, { task: 'theirs', completed: false });
